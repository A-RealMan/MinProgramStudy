...
exports.main = async(event, context) => {
  ...
  app.router('list', async(ctx, next) => {
    ...
    const blogList = await blogCollection.where(w).skip(event.start).limit(event.count).orderBy('createTime', 'desc').get().then((res) => {
      return res.data
    })
    ctx.body = blogList
  })
 ...
 app.router('detail', async(ctx, next) => {
    ...
    console.log('接收到的blogId:'+event.blogId)
  })
